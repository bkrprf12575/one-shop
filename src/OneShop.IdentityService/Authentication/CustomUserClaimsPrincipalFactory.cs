...
using OneShop.ServiceDefaults.Constants;
using System.Security.Claims;

namespace OneShop.IdentityService.Authentication
{
    public class CustomUserClaimsPrincipalFactory<TUser, TRole>(UserManager<TUser> userManager, RoleManager<TRole> roleManager, IOptions<IdentityOptions> options) 
        : UserClaimsPrincipalFactory<TUser, TRole>(userManager, roleManager, options) where TUser : IdentityUser<int> where TRole : IdentityRole<int>
    {
